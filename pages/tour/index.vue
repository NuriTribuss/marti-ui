        ...
        <div class="row" v-if="tours">
           <div class="col-lg-6" v-for="(tour,index) in tours" :key="index">
             <div class="tour-card card-item card-item-list ">
                    <div class="card-img">
                        <a :href="'/tour/'+tour.translate.url" class="d-block">
                            <img :src="'https://martireisen.at/'+tour.image" alt="Destination-img">
                        </a>
                    </div>
                    <div class="card-body ">
                        <h3 class="card-title"><a :href="'/tour/'+tour.translate.url">{{ tour.translate.name }}</a></h3>
                        <p class="card-meta font-size-12 line-height-20">{{ tour.departure_place }} > {{ tour.destination}}</p>
                        
                     
                        <div class="mt-0 mt-lg-4 pb-3 ">
                          <span class="price__num font-size-20 me-5 font-weight-bold text-color-9">$124.00</span>
                          <a :href="'/tour/'+tour.translate.url" class="float-end btn theme-btn-blue rounded-0  px-5 text-white">{{ $t['tour.look']}}</a>
                        </div>
                    </div>
                </div>
           </div>
...
export default {
  ...
  methods: {
    getData() {
      let vue = this;
      $fetch("/api/booking/tour/tour/?active=1").then(function (result) {
        if (!result.status) {
          return false;
        }
